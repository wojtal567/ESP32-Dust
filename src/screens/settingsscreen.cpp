...
void SettingsScreen::initialize() {
    ...
    m_backButton = createButton(m_screenContainer, NULL, 30, 15, 14, 10, [](lv_obj_t *obj, lv_event_t event) {
    ...
    m_wifiSettingsButton = createButton(m_screenContainer, NULL, 60, 60, 60, 38, [](lv_obj_t *obj, lv_event_t event) {
        if (event == LV_EVENT_CLICKED) {
    ...
    m_infoButton = createButton(m_screenContainer, m_wifiSettingsButton, 60, 60, 200, 38, [](lv_obj_t *obj, lv_event_t event) {
        if (event == LV_EVENT_CLICKED) {
    ...
    m_timeSettingsButton
        = createButton(m_screenContainer, m_wifiSettingsButton, 60, 60, 60, 140, [](lv_obj_t *obj, lv_event_t event) {
              if (event == LV_EVENT_CLICKED) {
                    lv_scr_load(timeSettingsScr);
                // screenManager.switchToScreen(BaseScreen::ScreenType::TIME_SETTINGS);
              }
          });
    ...
    m_timeSettingsButtonLabel = createLabel(m_screenContainer, m_wifiSettingsButtonLabel, 65, 200, "Time");

    m_samplingSettingsButton
        = createButton(m_screenContainer, m_wifiSettingsButton, 60, 60, 200, 140, [](lv_obj_t *obj, lv_event_t event) {
              if (event == LV_EVENT_CLICKED) {
    ...
    m_samplingSettingsButtonLabel = createLabel(m_screenContainer, m_wifiSettingsButtonLabel, 207, 200, "Sampling");
}